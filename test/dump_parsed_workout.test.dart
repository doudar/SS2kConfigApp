...
void main() {
  test('Parses test.zwo and dumps readable workout object', () async {
    ...
    if (!await zwoFile.exists()) {
      fail('test/test.zwo not found. Please run the JSON->ZWO test first.');
    }
    ...
    buffer.writeln('Total Duration: ${_formatDuration(
      workoutData.segments.fold(0, (sum, seg) => sum + seg.duration)
    )}\n');
    ...
    for (int i = 0; i < workoutData.segments.length; i++) {
      ...
      if (seg.isRamp) {
        buffer.write('Ramp from ${(seg.powerLow * 100).toStringAsFixed(1)}% to ${(seg.powerHigh * 100).toStringAsFixed(1)}% FTP');
      } else {
        buffer.write('Steady ${(seg.powerLow * 100).toStringAsFixed(1)}% FTP');
      }
      ...
      if (seg.textEvents.isNotEmpty) {
        ...
        for (final te in seg.textEvents) {
           ...
           buffer.writeln('      -> Msg: "${te.message}" at +${te.timeOffset}s');
        }
      }
      buffer.writeln();
    ...
    // 4. Write to file
    final dumpFile = File('test/dump_parsed_workout.txt');
    await dumpFile.writeAsString(buffer.toString());
    
    print('Parsed readable workout dump written to: ${dumpFile.absolute.path}');
  });
}
